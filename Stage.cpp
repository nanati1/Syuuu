...
void Stage::Update()
{
	...
	if (rand() % 60 == 0) { // 約1秒に1回
		int idx = rand() % enemy_.size();
		Enemy* shooter = enemy_[idx];
		if (shooter && shooter->IsAlive()) {
			// 敵の中心から弾を発射
			float ex = shooter->GetRect().x + shooter->GetRect().width / 2;
			float ey = shooter->GetRect().y + shooter->GetRect().height;
			EnemyBeam* beam = new EnemyBeam(ex, ey);
			beam->SetFired(true);
			newObjects.push_back(beam); // ←これが重要！
		}
	}
